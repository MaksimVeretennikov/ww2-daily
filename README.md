...
## Рубрики (несколько раз в неделю + итог недели)

Кроме ежедневного поста есть рубрики — **отдельные рутины, тот же репозиторий и
та же память** `state/history.json` (поэтому фото и личности не повторяются между
рубриками и дневным постом). Все рубричные рутины используют **то же окружение
«Default»**, что и дневная, — токен, network-allowlist и право push в `main` уже
настроены, заново ничего не нужно.

Каждая рутина: репозиторий `ww2-daily`, модель **Opus 4.8**, расписание **Weekly**
на нужный день, **20:00 МСК**, и промпт из одной строки. Чтобы не возиться с cron,
двойной слот «Личность» оформлен двумя недельными рутинами:

| Рутина | Навык | Расписание | Промпт |
|---|---|---|---|
| Личность (Вт) | `rubric-person` | Weekly · Вторник · 20:00 | «Запусти навык rubric-person, следуя инструкции навыка. Работай автономно.» |
| Кадр недели | `rubric-photo` | Weekly · Четверг · 20:00 | «Запусти навык rubric-photo, следуя инструкции навыка. Работай автономно.» |
| Личность (Сб) | `rubric-person` | Weekly · Суббота · 20:00 | «Запусти навык rubric-person, следуя инструкции навыка. Работай автономно.» |
| Итог недели | `weekly-digest` | Weekly · Воскресенье · 20:00 | «Запусти навык weekly-digest, следуя инструкции навыка. Работай автономно.» |

(Если хочешь одну рутину «Личность» на оба дня вместо двух — задай ей custom cron
`0 20 * * 2,6` через `/schedule update`.)

## Структура

```
ww2daily/            пакет с логикой (даты, источники, Commons, Telegram, X, state)
scripts/             fetch_sources / find_photo / publish / fetch_week (контекст недели)
.claude/skills/      навыки: daily-post, rubric-person, rubric-photo, weekly-digest
state/history.json   память канала (антиповторы тем, фото и личностей; поле kind)
